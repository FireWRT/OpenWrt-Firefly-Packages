...
#include <sys/ioctl.h>
#include <sys/types.h>
#include <generated/autoconf.h>  //kernel config

#define MTD_FACTORY     "/dev/mtd2"
#define LAN_OFFSET      0xE000
#define WAN_OFFSET      0xE006
#define RA0_OFFSET      0x0004
#define RAI0_OFFSET     0x8004
...
int mtd_read(char *side)
{
    ...
    if (0 == strcmp(side, "wan"))
        lseek(fd, WAN_OFFSET, SEEK_SET);
    else if( 0 == strcmp(side, "lan"))
        lseek(fd, LAN_OFFSET, SEEK_SET);
    else if( 0 == strcmp(side, "ra0"))
        lseek(fd, RA0_OFFSET, SEEK_SET);
    else if( 0 == strcmp(side, "rai0"))
        lseek(fd, RAI0_OFFSET, SEEK_SET);
    else
	return 0;
...
{
    printf("How to use:\n");
    printf("\tread:   %s r <lan|wan|ra0|rai0>\n", str[0]);
    printf("\twrite:  %s w <lan|wan|ra0|rai0> <MACADDR[0]> <MACADDR[1]> ...\n", str[0]);
}
